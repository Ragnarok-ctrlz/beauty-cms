    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="mb-8">
        ...
        <p class="mt-1 text-gray-600 dark:text-gray-300">
          Gérez vos sites web d'institut de beauté
        </p>
      </div>
      ...
      <div v-else>
        <!-- Aucun site existant -->
        <div
          v-if="userSites.length === 0"
          class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 text-center"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-24 w-24 mx-auto mb-4 text-gray-400"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
          ...
          <p class="text-gray-600 dark:text-gray-300 mb-6">
            Créez votre premier site d'institut de beauté en quelques minutes
          </p>
          <router-link
            to="/create-site"
            class="inline-flex items-center px-4 py-2 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-primary hover:bg-opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary"
          >
            Créer un site
          </router-link>
        </div>
              ...
              <div class="p-4">
                ...
                <div class="flex justify-between">
                  <router-link
                    ...
                    class="text-primary hover:text-opacity-80 text-sm font-medium"
                  >
                    Modifier
                  </router-link>
                  <button
...
const loadUserSites = async () => {
  try {
    ...
    const sitesQuery = query(collection(db, 'sites'), where('ownerId', '==', user.value.uid))
